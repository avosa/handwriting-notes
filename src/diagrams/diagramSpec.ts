...
export function toScene(spec: DiagramSpec): DiagramScene {
  ...
  switch (spec.kind) {
    case 'venn-subset': {
      ...
      shapes.push({
        type: 'label',
        at: { x: cx, y: cy - ry * 0.66 },
        text: spec.outer.label,
        color: spec.outer.color,
        size: 4,
      })
      shapes.push({
        type: 'label',
        at: { x: cx, y: cy + ry * 0.2 },
        text: spec.inner.label,
        color: spec.inner.color,
        size: 4,
      })
    ...
    case 'venn-overlap': {
      ...
      shapes.push({
        type: 'label',
        at: { x: lcx - rx * 0.6, y: cy - ry * 0.55 },
        text: spec.left.label,
        color: spec.left.color,
        size: 4,
      })
      shapes.push({
        type: 'label',
        at: { x: rcx + rx * 0.6, y: cy - ry * 0.55 },
        text: spec.right.label,
        color: spec.right.color,
        size: 4,
      })
      if (spec.middleLabel) {
        shapes.push({
          type: 'label',
          at: { x: (lcx + rcx) / 2, y: cy },
          text: spec.middleLabel,
          color: spec.middleColor ?? PURPLE,
          size: 3.8,
        })
      }
      shapes.push(caption(spec.caption))
    ...
    case 'venn-three': {
      ...
      shapes.push({
        type: 'label',
        at: { x: acx - rx * 0.55, y: topY - ry * 0.15 },
        text: spec.a.label,
        color: spec.a.color,
        size: 4.4,
      })
      shapes.push({
        type: 'label',
        at: { x: bcx + rx * 0.55, y: topY - ry * 0.15 },
        text: spec.b.label,
        color: spec.b.color,
        size: 4.4,
      })
